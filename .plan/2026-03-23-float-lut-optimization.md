...
Add a second LUT branch for float/16-bit data using **N = 1024 fixed buckets** that cover `colorScaleValueRange` linearly. Only `useHeatMap` benefits (the other modes — categorical, classes, single color — don't call chroma per pixel).

### Why N = 1024

- Power-of-two sizes are more efficient for memory alignment in the V8 engine and align with standard GPU texture practices. It also provides slightly better bit-depth coverage for higher-precision source data (10-bit/12-bit) without any performance cost.
...
- For slope (0–90°): each bucket = ~0.088° — imperceptible.
- For hillshade (0–255): each bucket = ~0.249 units — output is already integer, so effectively lossless.
- 1024 chroma calls per tile load is trivial vs 65,536.
- No `lutSize` option — 1024 is the right constant for all practical cases.
...
```typescript
    ...
    // Pre-compute N=1024 RGBA buckets covering colorScaleValueRange, then use a
    ...
    if (isFloatOrWide && optUseHeatMap && !optUseDataForOpacity) {
      ...
      const alphaByte = Math.floor(optAlpha * 2.55); // pre-calculate alpha
      ...
      for (let i = 0, pixel = 0; i < numPixels; i++) {
        ...
        pixel += samplesPerPixel; // explicit samplesPerPixel handling
      }
      return colorsArray;
    }
...
Pre-compute N=1024 RGBA buckets over colorScaleValueRange instead of
calling chroma.js per pixel. Reduces chroma calls from 65,536 to 1,024
...
1. **Power-of-Two LUT Size**: LUT size is now 1024 for memory alignment and GPU texture compatibility.
2. **Pre-calculate Alpha Byte**: Alpha is calculated once and stored in the LUT, not per pixel.
3. **Use `Math.floor` for Indexing**: LUT index calculation uses `Math.floor` for performance.
4. **Explicit `samplesPerPixel` Handling**: The pixel pointer is incremented by `samplesPerPixel` for multi-band COG compatibility.
5. **Guard for Zero-Span Ranges**: `rangeSpan` uses a fallback of 1 to avoid division by zero.
6. **Maintenance of `noData` and `NaN` Logic**: Guards for `NaN` and `noDataValue` are strictly before LUT math to preserve transparency/null handling.
